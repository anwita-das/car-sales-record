function CompanySalesTable({ data }) {
  return (
    <div className="max-h-[300px] overflow-y-auto border-none">
      <div className="min-w-full">
        <table className="w-full mx-auto border-collapse text-md mb-3">
          <thead className="sticky top-0 bg-[#efefeff7]">
            <tr>
              <th className="border border-[#ccc] px-2 py-1">Transmission</th>
          ...
          <tbody>
            {data.map((entry, index) => (
              <tr key={index}>
                <td className="border border-[#ccc] px-2 py-1">{entry.transmission}</td>
